...
{
    // Convert sexcoin:// to sexcoin:
    //
    //    Cannot handle this later, because sexcoin:// will cause Qt to see the part after // as host,
    //    which will lowercase it (and thus invalidate the address).
    if(uri.startsWith("sexcoin://"))
    {
        uri.replace(0, 11, "sexcoin:");
    }
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a sexcoin.desktop file to the autostart directory:
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Sexcoin-Qt") + " " + tr("version") + " " +
